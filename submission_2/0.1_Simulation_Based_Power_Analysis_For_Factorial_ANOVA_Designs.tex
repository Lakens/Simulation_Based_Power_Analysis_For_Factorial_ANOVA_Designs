...
\hypertarget{a-simple-scenario-for-power-analysis}{%
\section{A simple scenario for power analysis}\label{a-simple-scenario-for-power-analysis}}
...
\hypertarget{calculating-power-for-anovas}{%
\section{Calculating Power for ANOVAs}\label{calculating-power-for-anovas}}
...
If we assume that under the alternative hypothesis the true effect size is d = 0.5 or \(\eta_p^2\) = 0.0588, and data are collected from 80 participants in each condition, in the long run 8,816.02\% of the observed data will yield statistically significant results.

The \emph{t}-test can examine the difference between two means \((m_1 - m_2)\), and the \emph{F}-test computes the ratio of the between group variance and the within group variance.
For two independent groups, the \emph{t}-statistic can easily be translated to the \emph{F}-ratio \(F = t^2\).
...
G*Power (Faul et al., 2007), a very popular program for calculating sample size planning and power among social scientists, allows researchers to enter the effect size for the power analysis for ANOVA designs as either Cohen's f or partial eta-squared (\(\eta_p^2\)).
...
Users need to specify the design of the study by providing the number of levels for each factor and indicating whether the factor is measured between or within participants. We can create such a design using Superpower's ANOVA\_design function.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{design_result <-}\StringTok{ }\KeywordTok{ANOVA_design}\NormalTok{(}\DataTypeTok{design =} \StringTok{"2b"}\NormalTok{, }\DataTypeTok{n =} \DecValTok{80}\NormalTok{,}
...
\begin{figure}
\centering
\includegraphics{0.1_Simulation_Based_Power_Analysis_For_Factorial_ANOVA_Designs_files/figure-latex/mean-plot-orig-1.pdf}
\caption{\label{fig:mean-plot-orig}Vizualization for the means and standard deviations for the two conditions. Error bars represent one standard deviation.}
\end{figure}
...
The results of a simulation study will vary each time the simulation is performed (but can be made reproducible by specifying a \enquote{seed} number, e.g., \texttt{set.seed\ =\ 2019}).
...
If ANOVA\_power is being used, the more simulations that are performed, the more stable the results, but the longer the simulation will take.
...
Let's assume the mean enjoyment rating for the neutral voice condition falls either perfectly between the cheerful and sad conditions, or is equal to the cheerful condition. Again, we set this with ANOVA\_design.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{design_result_}\DecValTok{1}\NormalTok{ <-}\StringTok{ }\KeywordTok{ANOVA_design}\NormalTok{(}\DataTypeTok{design =} \StringTok{"3b"}\NormalTok{, }\DataTypeTok{n =} \DecValTok{80}\NormalTok{, }
...
If we assume the mean is equal to the cheerful condition, the power increases to 91.03v\%.
...
Instead of collecting three groups of participants, we only collect one group, and let this group evaluate the cheerful, neutral, and sad voice assistants. Again, we set this up with the ANOVA\_design function and notice the only differences are changing \texttt{3b} to \texttt{3w} and the correlation parameter, r = 0.5 is specificied within the function.
...
Taking into account the variance estimates from other groups through comparing the estimated marginal means (Lenth, 2019) (which the \emph{t}-tests do not do) can have power benefits (Maxwell et al., 2017), and these estimated marginal means comparisons can be accomplished by setting \texttt{emm\ =\ TRUE} within the ANOVA\_power or ANOVA\_exact functions. However, these depend on whether the homogeneity assumption is met, which may not be warranted in psychological research, and violations of the homogeneity assumption impact Type 1 error rates (Delacre, Lakens, Mora, \& Leys, 2018).
The degree to which the violation affects Type 1 error rate can be estimated with the ANOVA\_power function (see Assumptions section below)
