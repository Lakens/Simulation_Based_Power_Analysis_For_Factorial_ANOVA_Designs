...
The ANOVA_exact function simulates one function that has exactly the desired statistical properties, and computes power directly from the test results on this perfect dataset.
...
The simulation can be performed based on any design specified using the ANOVA_design function. 
...
```{r, scr2, fig.cap="Screenshot of ANOVA powerShiny app"}
knitr::include_graphics("screenshots/anova_power.png")
```
...
The output from ANOVA_exact and ANOVA_power is similar, and provides the statistical power for the ANOVA and all simple comparisons between conditions.
...
The same results are returned in the onine Shiny app, but here users can also choose a 'download PDF report' option to receive the results as a PDF file that can be saved to be included in a preregistration. 
...
Although an initial goal when performing an ANOVA might be to test the *omnibus null hypothesis*, which answers the question whether there are *any* differences between group means, we often want to know which conditions differ from each other.
...
Doing so can have power benefits [@maxwell_designing_2017], depending on whether the assumption of equal variances (also known as the homogeneity assumption) is met, which may not be warranted in psychological research [@delacre_why_2018]. 
...
Power analyses using a manually calculated Bonferoni correction can be performed with the ANOVA_exact function by specifying the adjusted alpha level, but the sequential Holm appraoch can only be performed in the ANOVA_power simulation approach.
