...
in the long run `r round(100*power_oneway_between(ANOVA_design(design = "2b", 80, c(1,0), 2, 0, c("condition", "cheerful", "sad"), FALSE))$power, 2)`% gives incorrect power estimate. Remove 100*? Also badly rounded. 
I have now added a footnote that we use sample means by default. You added 'sample' to means in Cohen's d's calculation. I removed it. Cohen's f was specified as population means - but I would suggest we also refer to sample means there (so observed scores). Then the following statement that they are directly related holds. Otherwise formally it does not hold (since we confuse population sd and sample sd). Right?
For a one-way between subjets design, do we provide Welch's f test? Not currently, right? Whereas it is good to do so if we assume variances are unequal. Would be another reason to use simulations (I think - not sure power formula exists). Maybe with Games-Howel post-hoc tests? (or just Welch's t-test for simple comparisons)
...
Whereas power analyses for individual comparisons are relatively easy to perform, power analyses for factorial ANOVA designs are a bigger challenge. There are many current software solutions that *can* calculate power for factorial ANOVAs [@faul_gpower_2007; @lang2017intermediately; @Campbell2012MorePower6F; @westfall2015pangea], but the options within these packages are often limited (e.g, Gpower has limited options for within subjects factors).
...
The effect size in the first study is sometimes refered to as a 'medium' effect size based on the benchmarks by @cohen_statistical_1988. 
...
```{r d-plot, fig.width=7, fig.height=4, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.cap="Distribution of Cohen's d under the null-hypothesis (grey curve) and alternative hypothesis assuming d = 0.5 (black curve) given n = 80."}
    N <- 80
    d <- 0.5
    p_upper<-.05
    p_lower<-0
...
```{r eta-plot, fig.width=7, fig.height=4, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.cap="Distribution of eta-squared under the null-hypothesis (grey curve) and alternative hypothesis assuming partial eta-squared = 0.0588 (black curve) given n = 80."}
...
To calculate the sample size required to reach a desired statistical power one has to specify the alternative hypothesis (the black curves in Figure \ref{fig:d-plot} and \ref{fig:eta-plot}), the sample size, and the alpha level.
Under the alternative hypothesis that the true effect size is d = 0.5 or $\eta_p^2$ = 0.0588, data are collected from 80 participants in each condition, and an alpha of 0.05 is used, in the long run `r round(100*power_oneway_between(ANOVA_design(design = "2b", 80, c(1,0), 2, 0, c("condition", "cheerful", "sad"), FALSE))$power, 2)`% of the observed data will yield statistically significant results.

**THIS SHOULD BE A BOX I THINK **
The *t*-test can examine the difference between two means $(m_1 - m_2)$, and the *F*-test computes the ratio of the between group variance and the within group variance.
For two independent groups, the *t*-statistic can easily be translated to the *F*-ratio $F = t^2$.
...
\end{equation}
If we have two groups with the sample means of 0 and 1, and the standard deviation is 2, Cohen's d is (1-0)/2, or 0.5.
The generalization of Cohen's d to more than two groups is Cohen's f, which is the standard deviation of the means divided by the standard deviation [@cohen_statistical_1988], or: 
\begin{equation}
...
G\*Power [@faul_gpower_2007], a very popular program for calculating sample size planning and power among social scientists, allows researchers to enter the effect size for the power analysis for ANOVA designs as either Cohen's f or partial eta-squared ($\eta_p^2$).
...
Based on $\lambda$ (the noncentrality parameter of the noncentral F distribution, which together with the degrees of freedom specifies the shape of the expected effect size distribution under a specified alternative hypothesis, Figure \ref{fig:eta-plot}) we can calculate the area under the curve that is more extreme than the critical effect size (i.e., Figure \ref{fig:eta-plot} to the right of the critical effect size).
Although these analytical functions are fast and easy, we can't always calculate power using these approaches, and have to rely on simulations instead. 
This becomes necessary when researchers use sequential approaches to corrections for multiple comparisons, such as the Holm procedure, or when test assumptions such as the sphericity assumption are violated. 
...
Users need to specify the design of the study by providing the number of levels for each factor and indicating whether the factor is measured between or within participants. We can create such a design using Superpower's ANOVA_design function.

```{r eval=F, echo=T}
design_result <- ANOVA_design(design = "2b", n = 80,
                              mu = c(1, 0), sd = 2,
...
```
```{r mean-plot-orig, fig.width=7, fig.height=4, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.cap="Vizualization for the means and standard deviations for the two conditions. Error bars represent one standard deviation."}

design_result$meansplot
```

For a visual confirmation of the input, a figure is created that displays the means and standard deviation (see Figure \ref{fig:mean-plot-orig}).
...
The results of a simulation study will vary each time the simulation is performed (but can be made reproducible by specifying a 'seed' number, e.g., `set.seed = 2019`).
...
If ANOVA_power is being used, the more simulations that are performed, the more stable the results, but the longer the simulation will take.
...
Now that the basic idea behind the simulation is clear, we can use simulations to explore how changes to the experimental design influence power, and answer some of the questions our hypothetical researcher is confronted with when designing a follow-up study.
We will first examine what happens if we add a third, neutral, condition to the design.
Let's assume the mean enjoyment rating for the neutral voice condition falls either perfectly between the cheerful and sad conditions, or is equal to the cheerful condition. Again, we set this with ANOVA_design.

```{r eval=F, echo=T}
design_result_1 <- ANOVA_design(design = "3b", n = 80, 
                                mu = c(1, 0.5, 0),
                                sd = 2, 
                                labelnames = c("condition", 
                                               "cheerful", 
...
If we assume the mean falls exactly between between the cheerful and sad conditions, the simulations show the statistical power for a 3-groups one-way ANOVA *F*-test is reduced to `r round(power_result_1$main_results$power[1],2)` %, and the effect size (partial eta-squared) is `r round(power_result_1$main_results$effect_size[1],2)`. 
If we assume the mean is equal to the cheerful condition, the power increases to `r round(power_result_2$main_results$power[1],2)`v%.
...
The exact effect of these three changes on the statistical power is difficult to anticipate from one ANOVA design to the next, but simulations can help illustrate the effects of these changes.
...
One cannot assume the effect size remains unchanged when the design changes, and it might make more sense to think about expected effects in terms of the pattern of means and variance across conditions.

# Power for Within-Subject Designs

What happens if we would perform the second study as a within-participants design?
Instead of collecting three groups of participants, we only collect one group, and let this group evaluate the cheerful, neutral, and sad voice assistants. Again, we set this up with the ANOVA_design function and notice the only differences are changing `3b` to `3w` and the correlation parameter, r = 0.5 is specificied within the function.
...
In the first case, we will only observe a main effect of voice, but in the other two scenarios there is an interaction effect between human-likeness of the voice and the emotional tone of the voice. We specify the design with the ANOVA_design function.
...
design_result_cross_80$meansplot
# set.seed(manuscript_seed)
...
Once we make that change, power with 40 participants per condition is `r power_result_cross_40$main_results$power[3]`%.
...
Taking into account the variance estimates from other groups through comparing the estimated marginal means [@lenthemmeans] (which the *t*-tests do not do) can have power benefits [@maxwell_designing_2017], and these estimated marginal means comparisons can be accomplished by setting `emm = TRUE` within the ANOVA_power or ANOVA_exact functions. However, these depend on whether the homogeneity assumption is met, which may not be warranted in psychological research, and violations of the homogeneity assumption impact Type 1 error rates [@delacre_why_2018]. 
The degree to which the violation affects Type 1 error rate can be estimated with the ANOVA_power function (see Assumptions section below)
...
# saveRDS(submission_2/sim_data/power_result_holm_correction, file = "power_result_holm_correction.rds")
...
And we perform a Monte Carlo simulation to obtain the power analysis. Please note that the ANOVA_power function must used for situations where we want to simulate power when multiple comparisons corrections are applied.
...
We see power for the interaction is reduced from `r power_result_cross_40$main_results$power[3]`% without applying a correction, to `r power_result_holm_correction$main_results$power[3]`% after correcting for multiple comparisons among the *F*-tests.
...
It is often advisable to preregister specific comparisons or tests that are of interest to a researcher.
...
#saveRDS(submission_2/sim_data/power_result_holm_correction, file = "power_result_holm_correction.rds")
power_result_s1 <- readRDS(file = "sim_data/power_result_s1.rds")

#knitr::kable(design_result$cor_mat, format = "latex", caption = "Table 1. Correlation Between Levels")%>%
#  kable_styling(latex_options = "hold_position")

```

An inquiring researcher may wonder, how much will this violation inflate the type 1 error rate? We can simulate an example (below) wherein there are no differences between the repeated measures and varying corrlations between the levels (r = .05-.90) of repeated measures. Again, we set this up in the ANOVA_design function.
...
The ANOVA_power function automatically stores plots for *p*-value distributions for each test in the simulation.
In Figure \ref{fig:p-plot} we see that for the initial 2 group between-participant design most *p*-values fall below the alpha level (in the figure, the vertical line as 0.05). We can produce by calling the stored results from the ANOVA_power function and selecting the plot1 object.
...
As power is increased the p-value distribution becomes steeper, and when there is no effect the *p*-values are uniformly distributed. 
