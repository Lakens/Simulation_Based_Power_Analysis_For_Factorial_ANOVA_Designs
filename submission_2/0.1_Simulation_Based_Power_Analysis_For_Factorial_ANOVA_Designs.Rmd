...
# A simple scenario for power analysis
...
# Calculating Power for ANOVAs

Before we directly answer these questions, let's consider some of the basic concepts of statistical power and how power calculations are typically performed.
...
When the observed data are more extreme than a critical value, the test result is statistically significant.
...
Cohen's d, a standardized effect size, is calculated by dividing the difference between sample means by the standard deviation, or 
\begin{equation}
...
Based on $\lambda$ (which is the noncentrality parameter of the noncentral F distribution, Figure \ref{fig:eta-plot}) and the critical test statistic (which specifies the part of the distribution that is more extreme than the test statistic needed for a statistically significant test result) we can calculate how much of the distribution under the alternative hypothesis will be statistically significant in the long run (i.e., the area under the black curve in Figure \ref{fig:eta-plot} to the right of the critical effect size). 
